...
export class RtBasicParser {
        ...
        for (let i = 0; i < document.lineCount; i++) {
            ...
            if (text.toLowerCase().startsWith('global dim')) {
                // 匹配数组变量: Global Dim varName(size) [As type]
                                    ...
                                    const globalMatch = text.match(/global\s+dim\s+([a-zA-Z_][a-zA-Z0-9_]*\s*(?:,\s*[a-zA-Z_][a-zA-Z0-9_]*)*)(?:\s+as\s+([a-zA-Z_][a-zA-Z0-9_]*))?/i);
                                    if (globalMatch) {
                            ...
                            // 解析局部变量
                                ...
                                const localMatch = text.match(/local\s+([a-zA-Z_][a-zA-Z0-9_]*\s*(?:,\s*[a-zA-Z_][a-zA-Z0-9_]*)*)(?:\s+as\s+([a-zA-Z_][a-zA-Z0-9_]*))?/i);
                                if (localMatch && currentSub) {
                            ...
                            // 解析文件变量
                                ...
                                const dimMatch = text.match(/dim\s+([a-zA-Z_][a-zA-Z0-9_]*\s*(?:,\s*[a-zA-Z_][a-zA-Z0-9_]*)*)(?:\s+as\s+([a-zA-Z_][a-zA-Z0-9_]*))?/i);
                                if (dimMatch) {
            ...
            // 解析结构体成员
                ...
                const memberMatch = text.match(/dim\s+([a-zA-Z_][a-zA-Z0-9_]*\s*(?:,\s*[a-zA-Z_][a-zA-Z0-9_]*)*)(?:\s+as\s+([a-zA-Z_][a-zA-Z0-9_]*))?/i);
                if (memberMatch) {
                    const memberNames = memberMatch[1].split(',').map(name => name.trim());
    ...
    public getSymbolAtPosition(document: vscode.TextDocument, position: vscode.Position): vscode.SymbolInformation | undefined {
